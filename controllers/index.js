...
const { customers: { findCustomer }, sessions: { createEditPaymentSession }, paymentMethods: { findPaymentMethod } } = require('../services/stripe')
...
const dashboard = async (req, res) => {
    try {
        ...
        if (SUBSCRIPTION) {
            stripe.session = await createEditPaymentSession(req.user.stripe_id)
            ...
            membershipDetails.dateNextPayment = await transformDate(new Date(SUBSCRIPTION.current_period_end * 1000))
            ...
            membershipDetails.dateCreated = req.user.date_created
                ? await transformDate(new Date(req.user.date_created))
                : undefined
...
const sendSupport = async (req, res) => {
    try {
        ...
        return res.status(400).send('Route \'/send-support\': ' + e.message)
    }
}
