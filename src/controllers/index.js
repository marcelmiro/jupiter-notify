...
const { findDiscordUser, inviteDiscordUser } = require('../services/discord/utils')
const { inStock, getCurrencySymbol } = require('../utils')
...
const dashboard = async (req, res) => {
    try {
        ...
        if (!ROLE) {
            ...
            if (
                !ROLE ||
                !await addUser(req.user.user_id, ROLE.role_id)
            ) return res.redirect('/')
        ...
        if (SUBSCRIPTION) {
            ...
            membershipDetails.dateNextPayment = !SUBSCRIPTION.cancel_at_period_end
                ? (parseInt(SUBSCRIPTION.current_period_end) * 1000 || 'null')
                : '-'
            membershipDetails.dateCreated = parseInt(SUBSCRIPTION.created) * 1000 || 'null'
            ...
            membershipDetails.dateCreated = parseInt(ROLE.created) || parseInt(req.user.created) || 'null'
...
module.exports = { index, dashboard, join, terms }
