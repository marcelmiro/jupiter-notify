...
const {
    listRoleIdsFromPlans, listCurrenciesFromPlans, listCurrenciesFromRoleId,
    findPlan, findPlanByRoleAndCurrency
} = require('../database/repositories/plans')
const { addUser, deleteUser } = require('../services/users')
const {
...
const pay = async (req, res) => {
    try {
        ...
        if (!currency) {
            const PLAN_CURRENCIES = plan
                ? await listCurrenciesFromRoleId(plan)
                : await listCurrenciesFromPlans()
...
const cancelMembership = async (req, res) => {
    try {
        ...
        console.log(`User '${req.user.username}' has canceled its '${ROLE.name.toLowerCase()}' license.`)
...
const transferMembership = async (req, res) => {
    try {
        ...
        if (
            !await addUser(userId, ROLE.role_id) ||
        ...
        if (EVENT.type === 'checkout.session.completed') {
            ...
            if (SESSION.mode === 'subscription') {
                ...
                if (!(await findUserRole(USER.user_id))) {
                    ...
                    if (!PLAN?.role_id) console.error(`No plan found on checkout.session.completed. PLAN_ID = ${PLAN_ID}.`)
                    if (!await addUser(USER.user_id, PLAN.role_id)) return
                }
