...
const pay = async (req, res) => {
    try {
        ...
        let { planId, plan, currency, password } = req.query
        try {
            await Joi.object().keys({
                planId: Joi.string(),
                plan: [Joi.number(), Joi.string()],
                currency: Joi.string()
            }).validateAsync({ plan, currency })
        } catch (e) { return res.redirect('/stripe/fail') }

        let PLAN
        if (planId) PLAN = await findPlan(planId)
        ...
        if (!PLAN) {
            ...
            if (ROLE?.role_id) PLAN = await findPlanByRoleAndCurrency(ROLE.role_id, currency)
        }

        if (!PLAN) return res.redirect('/stripe/fail')

        // Check if plan is for weekly or monthly subscriptions. If not, check for password.
        // This means that weekly and monthly plans don't require passwords to work.
        let trial
        if (PLAN.currency.includes('weekly')) {
            try {
                await Joi.number().required().validateAsync(process.env.weeklies)
            } catch (e) {
                delete process.env.weelies
                return res.redirect('/stripe/oos')
            }
            ...
            if (process.env.weeklies < 1) {
                ...
                process.env.weeklies = (process.env.weeklies - 1).toString()
            }

            trial = 7
        } else if (PLAN.currency.includes('monthly')) {
            try {
                await Joi.number().required().validateAsync(process.env.monthlies)
            } catch (e) {
                delete process.env.monthlies
                return res.redirect('/stripe/oos')
            }

            if (process.env.monthlies < 1) {
                delete process.env.monthlies
                return res.redirect('/stripe/oos')
            } else {
                process.env.monthlies = (process.env.monthlies - 1).toString()
            }

            trial = 30
        } else if (process.env.PASSWORD && process.env.PASSWORD !== password) {
            console.debug('Pay controller: Need (correct) password to buy subscription.')
            return res.redirect('/')
        }
...
const oos = async (req, res) => {
    try {
        res.render('response', { status: 'oos' })
    } catch (e) {
        console.error(e)
        res.redirect('/')
    }
