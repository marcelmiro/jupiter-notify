...
if (ENCRYPTION_KEY.length !== 32) {
    console.error('encryption.js: Encryption key is not a 32 character string.')
    process.exit(1)
} else ENCRYPTION_KEY = Buffer.from(ENCRYPTION_KEY, BUFFER_ENCRYPTION)

if (!KEY_LENGTH) {
    console.error('encryption.js: Key length is not a number.')
    process.exit(1)
}
if (!IV_LENGTH) {
    console.error('encryption.js: IV length is not a number.')
    process.exit(1)
}
