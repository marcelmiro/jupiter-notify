...
// AVATAR EVENTS
const image = document.querySelector('.dashboard__avatar .image')
const lowRes = document.querySelector('.dashboard__avatar .low-res')
const highRes = document.querySelector('.dashboard__avatar .high-res')
let highResComplete = false
const loadImage = url => {
    if (highResComplete) return
    image.style.backgroundImage = 'url(' + url + ')'
}
lowRes.addEventListener('error', () => {
    loadImage('https://cdn.discordapp.com/embed/avatars/1.png?size=256')
})
if (lowRes.complete) loadImage(lowRes.src)
else lowRes.addEventListener('load', () => loadImage(lowRes.src))
if (highRes.complete) {
    loadImage(highRes.src)
    highResComplete = true
} else {
