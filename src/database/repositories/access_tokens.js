...
const findAccessToken = async userId => {
    ...
    return (
        await client.query('SELECT * FROM access_tokens WHERE user_id = $1 LIMIT 1', [userId])
    ).rows[0]
}
...
const findAccessTokenByToken = async token => {
    ...
    return (
        await client.query('SELECT * FROM access_tokens WHERE access_token = $1 LIMIT 1', [token])
    ).rows[0]
}
...
const insertAccessToken = async userId => {
    ...
    return await client.query(
        ...
        [
            userId,
            uuidv4(),
            uuidv4().slice(0, parseInt(process.env.ENCRYPTION_IV_LENGTH)),
            new Date().getTime()
        ]
    )
}
