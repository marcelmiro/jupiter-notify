'use strict'
const { getRelease, deleteRelease } = require('../../services/releases')

module.exports = async ({ io, socket }) => {
    try {
        if (!socket.request.role?.['create_releases']) return socket.emit('send-error', 'You don\'t have permission to delete a release.')

        if (!(await getRelease())) return socket.emit('send-error', 'Release doesn\'t exist.')
        ...
        console.error('Socket \'delete-release\': ' + e.message)
        socket.emit('send-error', 'Error in socket \'delete-release\'.')
    }
}
