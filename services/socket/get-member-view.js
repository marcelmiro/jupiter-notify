...
const { customers: { findCustomer }, invoices: { findInvoice, findNextInvoice } } = require('../../services/stripe')
const { findDiscordUser } = require('../../services/discord/utils')
const { transformDate } = require('../../utils')

module.exports = async ({ socket, userId }) => {
    try {
        if (!socket.request.role?.['view_members']) return socket.emit('send-error', 'You don\'t have permission to view a member\'s data.')
        ...
        if (SUBSCRIPTION) {
            ...
            const LAST_INVOICE = SUBSCRIPTION.latest_invoice ? await findInvoice(SUBSCRIPTION.latest_invoice) : undefined
            if (LAST_INVOICE) {
        ...
        console.error('Socket \'get-member-view\': ' + e.message)
        socket.emit('send-error', 'Error in socket \'get-member-view\'.')
    }
}
